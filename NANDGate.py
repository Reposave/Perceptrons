...
if __name__ == '__main__':
	...
	if generate_validation_set:
		...
		for i in range(num_train):
			...
			validate_labels.append(0.0 if validate_examples[i][0] > 0.75 and validate_examples[i][1] > 0.75 else 1.0)
	...
	print(NAND.weights)
	valid_percentage = NAND.validate(validate_examples, validate_labels, verbose=True)
	...
	while valid_percentage < 0.98: # We want our Perceptron to have an accuracy of at least 80%
		...
		NAND.train(training_examples, training_labels, 0.2)  # Train our Perceptron
